...
class SaveDesignBySums extends BaseGrid
{
    protected
        $name,
        $id,
    ...
    protected function execute()
    {
        ...
        foreach ($this->cells as $cell) {
            if (empty($cell['is_data'])) {
                if (!empty($cell['display'])) {
                    $x = empty($cell['display'][0]) ? 0 : $cell['display'][0];
                    $y = empty($cell['display'][1]) ? 0 : $cell['display'][1];
                    $insert[] = '(' . $id . ',' . $cell['row'] . ',' . $cell['col'] . ',' . $y . ',' . $x . ')';
                    $cellKeyArr[] = "$y/$x";
                } else {
                    $cellKeyArr[] = '0/0';
                }
        ...
        $this->exec($sql, true, [implode(',', $cellKeyArr), $id]);
    }
